...
test('probe waits for complete reply before choosing the first wrapper', async () => {
  ...
  const probeWrites = writes.slice();
  assert.ok(probeWrites.length > 1, 'probe is typed one code point per write');
  assert.ok(!probeWrites.join('').includes('printf success'));
  pty.emit('data', `${job.marker}_P:fi`);
  assert.equal(writes.length, probeWrites.length, 'wrapper must wait for the full probe reply');
  pty.emit('data', `sh\r\n${job.marker}_Q`);
  const wrapperWrites = writes.slice(probeWrites.length);
  assert.ok(wrapperWrites.length > 1, 'wrapper is typed one code point per write');
  assert.ok(wrapperWrites.join('').includes('function __ncmcp_int'));
